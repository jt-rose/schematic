defmodule Schematic.SqlTriggersTest do
  ...
  describe "sql_triggers" do
    ...
    test "list_sql_triggers/0 returns all sql_triggers" do
      ...
      assert Triggers.list_sql_triggers() == [sql_trigger]
    end

    test "get_sql_trigger!/1 returns the sql_trigger with given id" do
      sql_trigger = sql_trigger_fixture()
      assert Triggers.get_sql_trigger!(sql_trigger.id) == sql_trigger
    end
    ...
    test "create_sql_trigger/1 with valid data creates a sql_trigger" do
      ...
      assert {:ok, %SqlTrigger{} = sql_trigger} = Triggers.create_sql_trigger(valid_attrs)
    ...
    test "create_sql_trigger/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Triggers.create_sql_trigger(@invalid_attrs)
    end
    ...
    test "update_sql_trigger/2 with valid data updates the sql_trigger" do
      ...
      assert {:ok, %SqlTrigger{} = sql_trigger} =
               Triggers.update_sql_trigger(sql_trigger, update_attrs)
    ...
    test "update_sql_trigger/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Triggers.update_sql_trigger(sql_trigger, @invalid_attrs)

      assert sql_trigger == Triggers.get_sql_trigger!(sql_trigger.id)
    end

    test "delete_sql_trigger/1 deletes the sql_trigger" do
      sql_trigger = sql_trigger_fixture()
      assert {:ok, %SqlTrigger{}} = Triggers.delete_sql_trigger(sql_trigger)
      assert_raise Ecto.NoResultsError, fn -> Triggers.get_sql_trigger!(sql_trigger.id) end
    end

    test "change_sql_trigger/1 returns a sql_trigger changeset" do
      sql_trigger = sql_trigger_fixture()
      assert %Ecto.Changeset{} = Triggers.change_sql_trigger(sql_trigger)
    end
  end
end
