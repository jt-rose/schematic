defmodule SchematicWeb.CoreComponents do
  @moduledoc """
  ...
  Icons are provided by [heroicons](https://heroicons.com). See `core_icon/1` for usage.
  """
  use Phoenix.Component

  alias Phoenix.LiveView.JS
  import SchematicWeb.Gettext
          ...
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            ...
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <.core_icon name="hero-x-mark-solid" class="w-5 h-5" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        <.core_icon :if={@kind == :info} name="hero-information-circle-mini" class="w-4 h-4" />
        <.core_icon :if={@kind == :error} name="hero-exclamation-circle-mini" class="w-4 h-4" />
        <%= @title %>
      </p>
      <p class="mt-2 text-sm leading-5"><%= msg %></p>
      <button type="button" class="absolute p-2 group top-1 right-1" aria-label={gettext("close")}>
        <.core_icon name="hero-x-mark-solid" class="w-5 h-5 opacity-40 group-hover:opacity-70" />
      </button>
    </div>
    """
  end
  ...
  def flash_group(assigns) do
    ...
    <.flash
      ...
      hidden
    >
      Attempting to reconnect <.core_icon name="hero-arrow-path" class="w-3 h-3 ml-1 animate-spin" />
    </.flash>
    """
  end
  ...
  def error(assigns) do
    ...
    <p class="flex gap-3 mt-3 text-sm leading-6 text-rose-600 phx-no-feedback:hidden">
      <.core_icon name="hero-exclamation-circle-mini" class="mt-0.5 h-5 w-5 flex-none" />
      <%= render_slot(@inner_block) %>
    </p>
    """
  end
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      ...
      >
        <.core_icon name="hero-arrow-left-solid" class="w-3 h-3" />
        <%= render_slot(@inner_block) %>
      </.link>
    </div>
    """
  ...
  ## Examples
      ...
      <.core_icon name="hero-x-mark-solid" />
      <.core_icon name="hero-arrow-path" class="w-3 h-3 ml-1 animate-spin" />
  """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SchematicWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SchematicWeb.Gettext, "errors", msg, opts)
    end
  end
