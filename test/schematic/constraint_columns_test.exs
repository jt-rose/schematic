defmodule Schematic.ConstraintColumnsTest do
  ...
  describe "constraint_columns" do
    ...
    test "list_constraint_columns/0 returns all constraint_columns" do
      ...
      assert ConstraintColumns.list_constraint_columns() == [constraint_column]
    end

    test "get_constraint_column!/1 returns the constraint_column with given id" do
      constraint_column = constraint_column_fixture()
      assert ConstraintColumns.get_constraint_column!(constraint_column.id) == constraint_column
    end
    ...
    test "create_constraint_column/1 with valid data creates a constraint_column" do
      ...
      assert {:ok, %ConstraintColumn{} = constraint_column} = ConstraintColumns.create_constraint_column(valid_attrs)
    ...
    test "create_constraint_column/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = ConstraintColumns.create_constraint_column(@invalid_attrs)
    end
    ...
    test "update_constraint_column/2 with valid data updates the constraint_column" do
      ...
      assert {:ok, %ConstraintColumn{} = constraint_column} = ConstraintColumns.update_constraint_column(constraint_column, update_attrs)
    ...
    test "update_constraint_column/2 with invalid data returns error changeset" do
      constraint_column = constraint_column_fixture()
      assert {:error, %Ecto.Changeset{}} = ConstraintColumns.update_constraint_column(constraint_column, @invalid_attrs)
      assert constraint_column == ConstraintColumns.get_constraint_column!(constraint_column.id)
    end

    test "delete_constraint_column/1 deletes the constraint_column" do
      constraint_column = constraint_column_fixture()
      assert {:ok, %ConstraintColumn{}} = ConstraintColumns.delete_constraint_column(constraint_column)
      assert_raise Ecto.NoResultsError, fn -> ConstraintColumns.get_constraint_column!(constraint_column.id) end
    end

    test "change_constraint_column/1 returns a constraint_column changeset" do
      constraint_column = constraint_column_fixture()
      assert %Ecto.Changeset{} = ConstraintColumns.change_constraint_column(constraint_column)
    end
  end
end
